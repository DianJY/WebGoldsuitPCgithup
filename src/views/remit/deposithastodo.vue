  ...
  <div>
          ...
          <el-table-column label="操作" width="290">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                @click="Remitaccount(scope.row.pId,scope.row.rId)"
                round
                size="small"
              >划账</el-button>
              <el-button
    ...
    <el-dialog title="划账信息" :visible.sync="dialogVisible2" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="TransferProjectData()">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //划账
    async TransferProjectData() {
      var obj = { rId: this.rId, eId: window.sessionStorage.getItem("eId") };
      const res = await TransferProject(obj);

      if (res === 0) return this.$message.error("数据出错！");



      this.$message({ message: "划账成功！", type: "success" });
