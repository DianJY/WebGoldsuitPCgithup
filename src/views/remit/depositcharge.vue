      ...
      <div class="bodyDiv">
        <el-table :data="tableData" stripe style="width: 100%">
          ...
          <el-table-column label="操作" width="380">
            <template slot-scope="scope">
              <el-button type="primary" @click="RemitPstate(scope.row)" round size="small">申请划账</el-button>
        ...
        <el-dialog title="驳回详细信息" :visible.sync="dialogVisible1" width="70%">
          <el-table :data="RemitRejectList" stripe style="width: 100%">
            ...
            <el-table-column prop="jDateTime" label="时间" width="300">
...
export default {
  ...
  methods: {
    ...
    async RemitPstate(row) {
      ...
      const state = await GetRemitInfo(row.pId);

      if (state.data == 0 || state.data.Data.ReState == "已驳回") {
        const res = await SaveRemit(Remit);
        if (res.data !== 1)
          return this.$message.error({ message: "数据出错！" });
