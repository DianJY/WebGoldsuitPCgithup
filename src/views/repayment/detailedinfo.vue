<template>
  <div>
    <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane label="还款审批日志" name="second3">
        <el-table :data="RepayApproval" stripe style="width: 100%">
          <el-table-column type="index" label="序号" width="180"></el-table-column>
...
import { GetInvestmentList, GetRepaymentInfo } from "../../network/pm";
import { GetRemitjournalList } from "../../network/remit";
import { GetRepayApprovalList } from "../../network/repaymentagency";
export default {
  created() {
    this.AraftsInfo();
    this.GetInvestmenData();
    this.GetRepayApprovalListData();
  },
  data() {
    return {
      activeName: "first",
  ...
  methods: {
    ...
    //查询还款审批审核表
    async GetRepayApprovalListData() {
      const res = await GetRepayApprovalList(this.pId);
      this.RepayApproval = res.data.Data
